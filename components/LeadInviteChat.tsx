  ...
  return (
    <div className="grid gap-3">
      <div className="grid max-h-64 gap-2 overflow-auto rounded-lg border border-line bg-slate-50 p-3">
        {messages.map((message, index) => (
          <p className={`m-0 rounded-lg border p-3 text-sm font-normal leading-6 whitespace-pre-wrap ${message.role === "user" ? "justify-self-end border-blue-200 bg-blue-50" : "border-line bg-white"}`} key={`${message.role}-${index}`}>
            <strong className="mb-1 block text-xs font-normal text-accent-strong">{message.role === "user" ? "You" : "Reachlyst AI"}</strong>
            {message.content}
          </p>
        ))}
        {isSending ? <p className="m-0 rounded-lg border border-line bg-white p-3 text-sm font-normal leading-6"><strong className="mb-1 block text-xs font-normal text-accent-strong">Reachlyst AI</strong>Polishing...</p> : null}
        <div ref={endRef} />
      </div>
      ...
      <div className="grid gap-2 md:grid-cols-[1fr_auto]">
        <textarea
          className="min-h-16 rounded-lg border border-line p-3 text-sm font-normal leading-6 text-ink outline-none transition placeholder:text-slate-400 focus:border-blue-300 focus:ring-4 focus:ring-blue-100"
