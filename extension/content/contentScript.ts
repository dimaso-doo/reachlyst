...
async function runSalesProfile() {
  ...
  const status = document.querySelector(".reachlyst-floating-chat .reachlyst-chat-status");
  if (status) status.textContent = "Profile context ready. Ask freely about fit, angle, message, or next step.";
}
...
document.addEventListener("scroll", (event) => {
  ...
  if (target instanceof Element && (target.closest(".reachlyst-floating-chat") || target.closest(".reachlyst-status") || target.closest(".reachlyst-lead-button"))) return;
  scheduleReachlystRun();
}, true);
