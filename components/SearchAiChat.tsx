  ...
  return (
    <section className="grid gap-4 rounded-lg border border-line bg-white p-5 shadow-[0_6px_18px_rgba(15,23,42,.04)]">
      <div>
        ...
        <p className="mt-1 text-sm font-semibold leading-6 text-muted">Use this as a sales strategy chat for ICP, fit rules, exclusions, message tone, and manual outreach copy.</p>
      </div>
      <div className="grid max-h-72 gap-2 overflow-auto rounded-lg border border-line bg-slate-50 p-3">
        ...
        {isSending ? <p className="m-0 rounded-lg border border-line bg-white p-3 text-sm font-semibold leading-6"><strong className="mb-1 block text-xs font-extrabold text-accent-strong">Reachlyst AI</strong>Thinking...</p> : null}
        <div ref={endRef} />
      </div>
      <div className="grid gap-2 md:grid-cols-[1fr_auto]">
        <textarea
          className="min-h-20 rounded-lg border border-line p-3 text-sm font-semibold leading-6 text-ink outline-none transition placeholder:text-slate-400 focus:border-blue-300 focus:ring-4 focus:ring-blue-100"
