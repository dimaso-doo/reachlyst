...
function openFloatingChat(lead) {
    ...
    const generateButton = chat.querySelector('[data-action="generate"]');
    if (generateButton)
        generateButton.textContent = lead.context === "messages" ? "Generate reply" : "Generate invite";
    const input = chat.querySelector(".reachlyst-chat-input");
    if (input)
...
async function generateInvite(lead, chat, button) {
    ...
    if (status)
        status.textContent = isMessages ? "Generating a copyable reply..." : "Generating a copyable invite...";
    ...
    renderThread(chat, lead);
    if (status)
        status.textContent = isMessages ? "Reply saved as a Reachlyst suggestion." : "Invite saved as a Reachlyst suggestion.";
...
async function runSalesProfile() {
    ...
    const status = document.querySelector(".reachlyst-floating-chat .reachlyst-chat-status");
    if (status)
        status.textContent = "Profile context ready. Ask for fit, angle, or a better invite.";
}
