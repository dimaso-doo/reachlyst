...
function visibleActionControl(element, card) {
    ...
    if (rect.left < cardRect.left + cardRect.width * 0.52)
        return false;
    if (rect.top < cardRect.top - 2 || rect.bottom > cardRect.bottom + 2)
        return false;
    const label = [
        element.textContent || "",
...
function actionClusterFor(control, card) {
    ...
    while (node && node !== card) {
        ...
        if (insideCard && controls.length >= 2 && controls.length <= 8)
            return node;
...
function findLeadActionsContainer(card) {
    if (!(card instanceof HTMLElement))
        return null;
    const cardRect = card.getBoundingClientRect();
    if (cardRect.width < 240 || cardRect.height < 56)
        return null;
...
function attachLeadButtons(leads) {
    ...
    for (const lead of leads) {
        ...
        if (button)
            activeKeys.add(button.dataset.reachlystKey);
    }
    document.querySelectorAll(".reachlyst-lead-button").forEach((button) => {
        if (!activeKeys.has(button.dataset.reachlystKey))
            button.remove();
    });
